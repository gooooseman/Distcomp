...
class TagModel:
    ...
    def update(self, tag_id, update_data):
...
class ReactionModel:
    ...
    def update(self, reaction_id, update_data):
        reaction = self.get_by_id(reaction_id)
        if reaction:
            reaction.update(update_data)
            return reaction
        return None
...
async def update_user(user_data: Dict[str, Any] = Body(...)):
    try:
        ...
        if "login" in user_data:
            if len(user_data["login"]) < 3 or len(user_data["login"]) > 50:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Login must be between 3 and 50 characters"
                )
...
async def create_tag(tag: Dict[str, Any] = Body(...)):
    try:
        if not all(k in tag for k in ["name", "newsId"]):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Missing required fields"
            )

        news_id = parse_id(tag.get("newsId"), "news")
...
async def update_tag(tag_data: Dict[str, Any] = Body(...)):
    try:
        if not all(k in tag_data for k in ["id", "name"]):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Missing required fields"
            )

        tag_id = parse_id(tag_data.get("id"), "tag")
        tag_name = tag_data.get("name", "").strip()

        tag = db_tags.get_by_id(tag_id)
        if not tag:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Tag not found"
        ...
        updated_tag = db_tags.update(tag_id, {"name": tag_name})
        return updated_tag
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
...
async def create_reaction(reaction: Dict[str, Any] = Body(...)):
    try:
        if not all(k in reaction for k in ["reactionType", "newsId", "userId"]):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Missing required fields"
            )

        news_id = parse_id(reaction.get("newsId"), "news")
        user_id = parse_id(reaction.get("userId"), "user")
...
async def update_reaction(reaction_data: Dict[str, Any] = Body(...)):
    try:
        if not all(k in reaction_data for k in ["id", "reactionType"]):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Missing required fields"
            )

        reaction_id = parse_id(reaction_data.get("id"), "reaction")
        reaction_type = reaction_data.get("reactionType")

        reaction = db_reactions.get_by_id(reaction_id)
        if not reaction:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Reaction not found"
        ...
        updated_reaction = db_reactions.update(reaction_id, {"reactionType": reaction_type})
