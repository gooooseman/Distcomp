package com.example.eger.dc.lab4.dao.impl

import com.example.eger.dc.lab4.bean.Label
import com.example.eger.dc.lab4.dao.LabelDao
import com.example.eger.dc.lab4.database.Labels
...
class LabelDaoImpl(private val connection: Connection) : LabelDao {
	init {
		...
		statement.executeUpdate(Labels.CREATE_TABLE_STICKERS)
	}
	...
	override suspend fun create(item: Label): Long = withContext(Dispatchers.IO) {
		val statement = connection.prepareStatement(Labels.INSERT_STICKER, Statement.RETURN_GENERATED_KEYS)
		statement.apply {
	...
	override suspend fun deleteById(id: Long): Int = withContext(Dispatchers.IO) {
		val statement = connection.prepareStatement(Labels.DELETE_STICKER)
	...
	override suspend fun getAll(): List<Label> = withContext(Dispatchers.IO) {
		...
		val statement = connection.prepareStatement(Labels.SELECT_STICKERS)
	...
	override suspend fun getById(id: Long): Label = withContext(Dispatchers.IO) {
		val statement = connection.prepareStatement(Labels.SELECT_STICKER_BY_ID)
		statement.setLong(1, id)
	...
	override suspend fun update(item: Label): Int = withContext(Dispatchers.IO) {
		val statement = connection.prepareStatement(Labels.UPDATE_STICKER)
		statement.apply {
			setString(1, item.name)
