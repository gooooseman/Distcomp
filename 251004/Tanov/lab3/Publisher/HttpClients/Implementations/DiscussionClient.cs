...
public class DiscussionClient : IDiscussionClient
{
    ...
    public async Task<MessageResponseDTO?> CreateMessageAsync(LabelRequestDTO post)
    {
        var httpClient = _factory.CreateClient(nameof(DiscussionClient));
    ...
    public async Task<MessageResponseDTO?> UpdateMessageAsync(LabelRequestDTO post)
    {
        var httpClient = _factory.CreateClient(nameof(DiscussionClient));
        var postJson = JsonSerializer.Serialize(post);
        var content = new StringContent(postJson, Encoding.UTF8, "application/json");
