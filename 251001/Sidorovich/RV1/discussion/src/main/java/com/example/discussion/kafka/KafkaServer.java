...
public class KafkaServer {
    ...
    public final KafkaController commentKafkaController;
    ...
    private void getMessage(ConsumerRecord<String, String> record){
        ...
        try {
            switch (message.operation()) {
                case GET_ALL -> response = new MessageData(message.operation(), commentKafkaController.getAll());
                case CREATE ->
                        response = new MessageData(message.operation(), Collections.singletonList(commentKafkaController.create(message.requestTO())));
                case UPDATE ->
                        response = new MessageData(message.operation(), Collections.singletonList(commentKafkaController.update(message.requestTO())));
                case GET_BY_ID ->
                        response = new MessageData(message.operation(), Collections.singletonList(commentKafkaController.getById(message.itemId())));
                case DELETE_BY_ID -> commentKafkaController.delete(message.itemId());
            }
            if(response == null){
