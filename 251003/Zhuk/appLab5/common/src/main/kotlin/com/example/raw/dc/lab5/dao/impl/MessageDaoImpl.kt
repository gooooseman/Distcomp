package com.example.raw.dc.lab5.dao.impl

import com.datastax.oss.driver.api.core.CqlSession
import com.example.raw.dc.lab5.bean.Message
import com.example.raw.dc.lab5.dao.MessageDao
import com.example.raw.dc.lab5.database.Messages
import com.example.raw.dc.lab5.id
import com.example.raw.dc.lab5.testViaRedis
...
class MessageDaoImpl(private val session: CqlSession) : MessageDao {
	init {
		session.execute(
			...
			(
				...
				${Messages.COLUMN_STORY_ID} bigint,
		...
		return@withContext try {
			...
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = $id
					...
					AND ${Messages.COLUMN_STORY_ID} = ${entity.storyId};
				""".trimIndent()
			)
	...
	override suspend fun getAll(): List<Message> = withContext(Dispatchers.IO) {
		...
		for (row in resultSet.all()) {
			...
			val storyId = row.getLong(Messages.COLUMN_STORY_ID.toString())
		...
		return@withContext try {
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = ${item.id}
					...
					AND ${Messages.COLUMN_STORY_ID} = ${item.storyId};
				""".trimMargin()
			)
