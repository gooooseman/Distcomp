...
import com.example.eger.dc.lab4.dao.MessageDao
import com.example.eger.dc.lab4.database.Messages
...
class MessageDaoImpl(private val session: CqlSession) : MessageDao {
		...
		return@withContext try {
			...
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = $id
					...
					AND ${Messages.COLUMN_STORY_ID} = ${entity.issueId};
				""".trimIndent()
			)
	...
	override suspend fun getAll(): List<Message> = withContext(Dispatchers.IO) {
		...
		for (row in resultSet.all()) {
			...
			val issueId = row.getLong(Messages.COLUMN_STORY_ID.toString())
		...
		return@withContext try {
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = ${item.id}
					...
					AND ${Messages.COLUMN_STORY_ID} = ${item.issueId};
				""".trimMargin()
			)
