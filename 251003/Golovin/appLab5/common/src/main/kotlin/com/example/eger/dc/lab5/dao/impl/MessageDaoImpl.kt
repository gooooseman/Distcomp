...
import com.example.eger.dc.lab5.bean.Message
import com.example.eger.dc.lab5.dao.MessageDao
import com.example.eger.dc.lab5.database.Messages
...
class MessageDaoImpl(private val session: CqlSession) : MessageDao {
		...
		return@withContext try {
			...
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = $id
					...
					AND ${Messages.COLUMN_ISSUE_ID} = ${entity.storyId};
				""".trimIndent()
			)
	...
	override suspend fun getAll(): List<Message> = withContext(Dispatchers.IO) {
		...
		for (row in resultSet.all()) {
			...
			result.add(
				Message(
					id = id, storyId = issueId, content = content, country = country
				)
			)
		}
		...
		return@withContext try {
			session.execute(
				...
				WHERE ${Messages.COLUMN_ID} = ${item.id}
					...
					AND ${Messages.COLUMN_ISSUE_ID} = ${item.storyId};
				""".trimMargin()
			)
